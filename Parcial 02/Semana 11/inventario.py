...
class Inventario(Producto):  # Clase para gestionar el inventario de productos, hereda de la clase Producto
    ...
    def añadir_producto(self, producto):  # Método para añadir un nuevo producto al inventario
        while True:
            ...
            self.guardar_inventario_en_txt()
    ...
    def buscar_producto_por_nombre(self,
                                   nombre):  # Método para buscar un producto en el inventario utilizando su nombre
        for producto in self.lista_productos:
            if producto["nombre"].lower() == nombre.lower():
                print(
                    f"Producto encontrado: ID: {producto['ID']}, Nombre: {producto['nombre']}, Precio: {producto['precio']}, Cantidad: {producto['cantidad']}")
                return
    ...
    def guardar_inventario_en_txt(self,
                                  nombre_archivo="inventario.txt"):  # Método para guardar el inventario en un archivo TXT
        try:
            with open(nombre_archivo, "w", encoding="utf-8") as archivo:
                if not self.lista_productos:
                    archivo.write("El inventario está vacío.\n")
                else:
    ...
    def total_a_pagar(self):  # Nuevo: calcula el total (precio * cantidad) de todo el inventario
        total = sum(p["precio"] * p["cantidad"] for p in self.lista_productos)
        print(f"Total a pagar por todo el inventario: {total:.2f}")
...
while True:
    ...
    print("7. Ver total a pagar")
    print("8. Salir")

    opcion = input("Ingrese una opción (1-8): ")  # opciones para elegir
    if opcion == "1":
